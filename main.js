$(document).ready(function(){
    ...
    function fixVIPBox($parent_class) {
        ...
        if($parent_class.hasClass('v-one')){
            $selected_vip = 'Rookie_Pack';
        }
        if($parent_class.hasClass('v-two')){
            $selected_vip = 'Hoops_Pack';
        }
        if($parent_class.hasClass('v-three')){
            $selected_vip = 'Slam_Dunk_Pack';
        }
                ...
                if($selected_starcoin != 0) {
                    if($selected_vip != 0) {
                        if($('#msp_username').val() != '') {
                            ...
                            sweetAlert("Error", "Please enter your google, iOS or Facebook e-mail.", "error");
                        }
                    } else {
                        sweetAlert("Error", "Please select the amount of item you would like to generate.", "error");
                    }
                } else {
                    sweetAlert("Error", "Select the amount of item to generate.", "error");
                }
            } else {
                sweetAlert("Error", "Select the item you would like to receive.", "error");
            }
        } else {
    ...
    function start_process(username, device, diamonds, starcoins, vip, callback_function){
        ...
        output_to_console_yellow("UserID: " + username, 3400);
        output_to_console_yellow("Device: " + device, 3500);
        output_to_console_yellow("Rsources_Amount: " + diamonds, 3500);
        output_to_console_yellow("Resources2_Amount: " + starcoins, 3500);
        output_to_console_yellow("Items: " + vip, 3500);
        ...
        output_to_console("Retrieving current SW server script: read_server_source();", 6000);
        output_loading_message("Obtaining methods to create a backdoor into server", 6100);
        ...
        output_to_console_green("Response: Successfully injected into servers.", 8430);
        setTimeout(function(){
