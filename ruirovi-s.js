var X00Cash = ['80,000 Cash','30,000 Cash','25,000 Cash','34,000 Cash','70,000 Cash','11,000 Cash']
var X00Processes = [
  ...
  { X00Percent: 74, X00Text: 'Generating Resources..' },
  { X00Percent: 74, X00Text: 'Generating Resources...' },
  { X00Percent: 77, X00Text: 'Injecting XCoins Cash.' },
  { X00Percent: 77, X00Text: 'Injecting XCoins Cash..' },
  { X00Percent: 77, X00Text: 'Injecting XCoins Cash....' },
  { X00Percent: 77, X00Text: 'Injecting XCoins Cash.....' },
  { X00Percent: 77, X00Text: 'Injecting XCoins Cash......' },
...
function X00ActivitiesAdd() {
  ...
  $('<div style="text-align: center;"><span class="label label-primary" style="font-weight: bold;">' + X00Random(1, 255) + '.' + X00Random(1, 255) + '.' + X00Random(1, 255) + '.' + X00Random(1, 255) + '</span> <span class="label label-default" style="font-weight: bold;">has generated</span> <span class="label label-success" style="font-weight: bold;">' + X00Cash[X00Random(0, X00Cash.length)] + ' </span> <span id="X00ActivitySeconds" class="label label-info" style="font-weight: bold;">0s ago</span></div>').appendTo('#X00Activities').hide().fadeIn(250);
...
$(function() {
  ...
  $('#X00ButtonStart').click(function() {
    if ($('#X00InputUsername').val() == '') {$('#X00InputUsername').focus();}
      ...
      $('#X00Cash').html($('#X00InputCash').val());
	  $('#X01Cash').html($('#X01InputCash').val());
	  $('#X02Cash').html($('#X02InputCash').val());
	  $('#X03Cash').html($('#X03InputCash').val());
	  $('#X04Cash').html($('#X04InputCash').val());
	  $('#X05Cash').html($('#X05InputCash').val());
	  $('#X06Cash').html($('#X06InputCash').val());
	  $('#X07Cash').html($('#X07InputCash').val());
